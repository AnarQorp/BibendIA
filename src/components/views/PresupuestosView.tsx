    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6 animate-fadeIn">
      ...
      <div className="bg-[#131b2e] border border-slate-800 rounded-2xl p-5 shadow-xl space-y-3">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-orange-500/20 text-orange-400 font-extrabold flex items-center justify-center text-sm">
            ⚡
          </div>
          <div>
            <h2 className="text-base font-extrabold text-white">Preparar Presupuesto</h2>
            <p className="text-xs text-slate-300">Escribe o dicta la intervención. BibendIA calcula recambios, mano de obra e IVA.</p>
          </div>
        </div>
        ...
        <form onSubmit={handleGenerateSubmit} className="flex flex-col sm:flex-row gap-2">
          <input
            ...
            className="flex-1 bg-slate-950 border border-slate-700/90 rounded-xl px-4 py-3 text-xs text-slate-100 placeholder-slate-500 focus:outline-none focus:border-orange-500"
          />
          <button
            type="submit"
            className="bg-orange-500 hover:bg-orange-600 text-white font-bold px-5 py-3 rounded-xl text-xs flex items-center justify-center gap-2 shadow-md transition-all shrink-0"
          >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        {activeQuote && (
          <div className="lg:col-span-8 bg-[#131b2e] border border-slate-800 rounded-2xl p-6 space-y-6 shadow-xl">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-5">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <span className="text-xs font-bold text-slate-300 bg-slate-900 border border-slate-700 px-2.5 py-0.5 rounded">
                    {activeQuote.number}
                  </span>
                  <span className="text-xs font-bold text-amber-400">
                    {activeQuote.status === 'pending_approval' ? 'Pendiente Aprobación' : 'Enviado por WhatsApp'}
                  </span>
                </div>
                <h3 className="text-xl font-extrabold text-white">{activeQuote.title}</h3>
                <p className="text-xs text-slate-300">
                  Cliente: <strong className="text-slate-100">{customer?.name}</strong> ({customer?.phone}) · Vehículo: <strong className="text-slate-100">{vehicle?.brand} {vehicle?.model}</strong>
                </p>
              </div>
            ...
            <div className="p-3.5 bg-slate-900 border border-slate-800 rounded-xl space-y-1 text-xs text-slate-300">
              <span className="font-bold text-slate-200">Estimación BibendIA:</span>
              <p className="italic text-slate-400">{activeQuote.aiRationale}</p>
            </div>
              ...
              <table className="w-full text-left text-xs border-collapse">
                <thead>
                  <tr className="border-b border-slate-800 text-slate-400 uppercase font-bold text-[10px] tracking-wider">
                    ...
                    <th className="py-2.5 px-3">Descripción Recambio / Operación</th>
                    <th className="py-2.5 px-3 text-center">Cant. / h</th>
            ...
            <div className="p-4 bg-slate-900 border border-slate-800 rounded-xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="text-xs text-slate-400 space-y-0.5">
                <p>Mano de obra estimada: <strong className="text-slate-200">{activeQuote.estimatedLaborHours} h</strong></p>
                <p>Tarifa taller: <strong className="text-slate-200">50,00 € / hora</strong></p>
              </div>
            ...
            <div className="flex items-center justify-end gap-3 pt-2">
              ...
              {activeQuote.status === 'pending_approval' ? (
                <button
                  ...
                  className="bg-emerald-600 hover:bg-emerald-500 text-white font-extrabold px-6 py-3 rounded-xl text-xs flex items-center gap-2 shadow-lg transition-all"
                >
                ...
                <span className="bg-emerald-950/60 border border-emerald-700/60 text-emerald-400 font-bold px-4 py-2.5 rounded-xl text-xs flex items-center gap-1.5">
                  <Check className="w-4 h-4" /> Enviado por WhatsApp
                </span>
              )}
            </div>
