...
describe('tenant pilot supervised mode', () => {
  it('hardens auto-low-risk actions through the normal approval result', () => {
    expect(evaluatePolicy(
      ...
      { requestedLevel: 'auto_low_risk', customerConfirmationRecorded: true, requiredFactsVerified: true, risk: 'low' },
  ...
  it('still requires explicit customer confirmation for appointment creation', () => {
    expect(evaluatePolicy(
      { operatingMode: 'pilot_supervised', policyVersion: 'pilot-v1' },
      { requestedLevel: 'customer_confirmed', customerConfirmationRecorded: false, requiredFactsVerified: true, risk: 'low' },
  ...
  it('allows a customer-confirmed action when facts and confirmation are recorded', () => {
    expect(evaluatePolicy(
      { operatingMode: 'pilot_supervised', policyVersion: 'pilot-v1' },
      { requestedLevel: 'customer_confirmed', customerConfirmationRecorded: true, requiredFactsVerified: true, risk: 'low' },
    ).effect).toBe('allow');
  });
});
