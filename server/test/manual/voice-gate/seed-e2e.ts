...
const hash=(v:string)=>createHash('sha256').update(v.toUpperCase().replace(/[^A-Z0-9]/g,'')).digest('hex');
...
try{
 ...
 await inTenantTransaction(pool,tenant,async c=>{
  ...
  await c.query("INSERT INTO customers(id,tenant_id,display_name) VALUES($1,$2,'Aitor Etxeberria')",[customer,tenant]);
  await c.query("INSERT INTO vehicles(id,tenant_id,plate_ciphertext,plate_hash,make,model) VALUES($1,$2,'1489 KMR',$3,'SEAT','León')",[vehicle,tenant,hash('1489 KMR')]);
